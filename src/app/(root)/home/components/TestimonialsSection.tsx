...
const TestimonialsSection = () => {
  ...
  useEffect(() => {
    const fetchFeaturedReviews = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/reviews?featured=true&approved=true');
  ...
  // Transform featured reviews to testimonials format
  const transformReviewsToTestimonials = (reviews: FeaturedReview[]) => {
    return reviews.map(review => ({
      car: review.car ? `${review.car.brand} ${review.car.model}` : 'Luxury Car',
      text: review.comment,
      name: review.userName,
      role: review.car ? `Customer, ${review.car.name}` : 'Customer',
    }));
  };

  // Fallback testimonials if no featured reviews are available
  ...
  // Use featured reviews if available, otherwise use fallback
  const testimonials = featuredReviews.length > 0 
    ? transformReviewsToTestimonials(featuredReviews)
      ...
      >
        <div className="container z-10 mx-auto">
          ...
          >
            ...
            <p className="text-center mt-5 opacity-75 text-foreground">Loading featured reviews...</p>
          </motion.div>
